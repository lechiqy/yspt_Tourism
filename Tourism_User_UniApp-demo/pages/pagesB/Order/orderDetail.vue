    ...
    <view class="order-detail-container">
        <view class="order-card">
            ...
            <view class="order-footer">
                <u-button v-if="orderDetail.Status === 'pending'" type="primary" size="medium" @click="handlePay">立即支付</u-button>
                <u-button v-if="orderDetail.Status === 'pending'" type="default" size="medium" @click="handleCancel">取消订单</u-button>
                <u-button v-if="orderDetail.Status === 'processing'" type="error" size="medium" @click="handleRefund">我要退款</u-button>
...
export default {
    ...
    methods: {
        ...
        handleRefund() {
            uni.showModal({
                title: '退款确认',
                content: '确定要申请退款吗？退款后订单将被取消。',
                success: async (res) => {
                    if (res.confirm) {
                        try {
                            await this.$api.refundOrder(this.orderDetail.OrderID);
                            uni.showToast({ title: '退款成功，订单已取消' });
                            this.loadData(this.orderId);
                        } catch (error) {
